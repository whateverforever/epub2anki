...
class Epub2Anki(toga.App):
    def startup(self):
        ...
        state = {"username": "Dummy", "epub_path": None, "anki_all_decks": ["No decks found"], "anki_selected_deck": None}
        ...
        wizard.style.update(flex=1, background_color="#dfd")
...
class FileChoosingScreen(ScreenWithState):
    def construct_gui(self):
        ...
        self.epub_file_btn = toga.Button(
            "Choose File", on_press=self.pressed_epub_btn
        )
        epub_box = toga.Box(children=[
            epub_label, self.epub_file_btn
        ...
        anki_box = toga.Box(children=[
            anki_label, anki_choice
        ], style=Pack(padding_bottom=5))
    ...
    def pressed_epub_btn(self, btn):
        ...
        if not epub_path.endswith(".epub"):
            app.main_window.error_dialog("Invalid File Suffix", "The file doesn't end in .epub, and so I won't take it. Fight me >:{")
            return
